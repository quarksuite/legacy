...
describe('Some sample workflows working with palettes', () => {
  test('Scenario: a single monochromatic color scheme', () => {
    ...
    expect(base.createTints().createShades(85, 2).paletteState).toStrictEqual([
      'rgb(220, 220, 220)',
      ['rgb(232, 232, 232)', 'rgb(243, 243, 243)', 'rgb(254, 254, 254)'],
      ['rgb(167, 167, 167)', 'rgb(87, 87, 87)']
    ]);
  });
  test("Scenario: Quarksuite's own color palette", () => {
    const palette = new Color('#348ec9')
      .createTriadicScheme()
      .schemeState.map((swatch, index) => {
        if (index === 0)
          return new Color(swatch).createTints().createShades(97, 2)
            .paletteState;
        return swatch;
      });
