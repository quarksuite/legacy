...
const parseSep = str => str.indexOf(',') > -1 ? ',' : ' ';
const hslData = hsl => {
...
const blend = (color, target, amount = 50, mode = 'logarithmic') => {
...
const complement = color => spin(color);
/**
...
const neutralize = color => blend(color, complement(color));
/**
...
const mix = (color, target, amount = 50) => blend(color, target, amount);
/**
...
const format = (color, format = 'rgb') => convert(color, format);

const complementary = color => [convert(color), convert(complement(color))];

const splitComplementary = (color, distance = 15, accented = false) => {
  const a = convert(color);
  const opposite = convert(complement(a)); // right of complement

  const b = convert(spin(opposite, 360 + distance)); // left of complement

  const c = convert(spin(opposite, 360 - distance));
  return accented ? [a, opposite, b, c] : [a, b, c];
};

const analogous = (color, distance = 15, accented = false) => {
...
const dualColor = (color, distance = 15) => {
  const a = convert(color, 'rgb');
...
const scheme = (color, type, options = {}) => {
...
const variants = (color, target, options = {}) => {
  // Initialize the options
  const {
    contrast = 97,
    limit = 2,
...
const system = family => fonts[family];
...
function goldenRatio() {
  ...
  const b = f[f.length - 1]; // Divide the largest numbers for accurate ratio.
...
const create = (base = 1, ratio = 'golden', limit = 6, invert = false) => {
  let r = 0; // Check if ratio is a named ratio or custom one
...
const modify = (scale, n, modifier) => scale.map(value => parseFloat(modifier(n, value).toPrecision(6)));
/**
...
const merge = (...scales) => {
  return Array.prototype.concat(...scales).sort((a, b) => a - b).filter((v, i, a) => a.indexOf(v) === i);
};
/**
...
const output = (scale, unit = 'rem', precision = 4) => {
...
const color = _objectSpread2({}, colorUtils);
const typography = _objectSpread2({}, typographyUtils);
const scale = _objectSpread2({}, scaleUtils);
